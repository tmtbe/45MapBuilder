...
package com.junkbyte.console.view 
{
	...
	public class MainPanel extends ConsolePanel {
		...
		public function MainPanel(m:Console) {
			...
			_cmdField.addEventListener(MouseEvent.MOUSE_DOWN, onCmdMouseDown);
			...
			this.stageText.addEventListener(KeyboardEvent.KEY_DOWN, commandKeyDown);
			this.stageText.addEventListener(KeyboardEvent.KEY_UP, commandKeyUp);
			this.stageText.addEventListener(FocusEvent.FOCUS_IN, updateCmdHint);
			this.stageText.addEventListener(FocusEvent.FOCUS_OUT, onCmdFocusOut);
			var stageTextViewPort:Rectangle = this.stageText.viewPort;
			if(!stageTextViewPort)
			{
				stageTextViewPort = new Rectangle();
			}
			stageTextViewPort.x = _cmdField.x;
			stageTextViewPort.y = _cmdField.y;
			stageTextViewPort.width = _cmdField.width;
			stageTextViewPort.height = _cmdField.height;
			this.stageText.color = _cmdField.defaultTextFormat.color;
			this.stageText.fontSize = _cmdField.defaultTextFormat.size;
		...
		private function onCmdMouseDown(e:MouseEvent):void 
		{
			this.stageText.visible = true;
			this.stageText.stage = stage;
			this.stageText.viewPort.x = _cmdField.x;
			this.stageText.viewPort.y = _cmdField.y;
			this.stageText.assignFocus();
		}
		protected function stageText_changeHandler(event:Event):void
		{
			_cmdField.text = stageText.text;
		}
		/**
		 * @private
		 */
		...
		protected function linkHandler(e:TextEvent):void{
			...
			if(t == "pause"){
				...
				if (ind >= 0) {
					...
					this.stageText.text = t.substring(ind+1);
				}
		...
		private function commandKeyUp(e:KeyboardEvent):void{
			if( e.keyCode == Keyboard.ENTER){
				...
				if (stage) this.stageText.visible = false;
				...
				if(_cmdsInd<(_cmdsHistory.length-1)){
					...
					this.stageText.text = _cmdsHistory[_cmdsInd];
					setCLSelectionAtEnd();
				}else{
		...
		private function setCLSelectionAtEnd():void{
			...
			stageText.selectRange(_cmdField.text.length, _cmdField.text.length);
		}
		...
		public function set commandLine(b:Boolean):void{
			if(b){
				...
				_cmdBG.visible = true;
				if(visible) this.stageText.visible = true;
			}else{
				_cmdField.visible = false;
				_cmdPrefx.visible = false;
				_cmdBG.visible = false;
		...
		{
			super.visible = value;
			if (!value) this.stageText.visible = false;
		}
	}
}