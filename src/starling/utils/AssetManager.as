                                      ...
                                      onProgress:Function, onComplete:Function):void
        {
            ...
            function onUrlLoaderComplete(event:Event):void
            {
                ...
                switch (extension)
                {
					...
					case "map":
						bytes.position = 0;
						map = bytes.readObject() as MapInfo;
						onComplete();
						break;
					default:
            ...
            function onLoaderComplete(event:Event):void
            {
				...
				var cWidth:int = (content as Bitmap).width;
				var cHeight:int = (content as Bitmap).height;
				if (map.mapBackName == name)
				{
					var rect:Rectangle = new Rectangle(0, 0, map.mapClipWidth, map.mapClipHeight);
					var icout:int = Math.ceil((content as Bitmap).width / rect.width);
					var jcout:int = Math.ceil((content as Bitmap).height / rect.height);
					for (var i:int = 0; i <icout; i++)
					{
						for (var j:int = 0; j < jcout; j++)
						{
							rect.x = rect.width * i;
							rect.y = rect.height * j;
							var lbitmapData:BitmapData = new BitmapData(Math.min(rect.width,cWidth-rect.x), Math.min(rect.height,cHeight-rect.y), true, 0);
							lbitmapData.copyPixels((content as Bitmap).bitmapData, rect, new Point());
							addTexture("&clip_"+name+"_"+i.toString()+"_"+j.toString(), Texture.fromBitmapData(lbitmapData, mGenerateMipmaps, false, mScaleFactor));
